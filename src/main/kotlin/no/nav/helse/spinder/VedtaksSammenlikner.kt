...
fun sammenliknVedtaksPerioder(
    ...
    val årsinntekt = årsinntekt(infotrygd.arbeidsforholdListe)
...
fun slåSammenInfotrygdVedtak(vedtaksListe: List<InfotrygdVedtak>): Either<VedtaksSammenlikningsFeil, InfotrygdVedtak> {
    ...
    for (i in 0..sortertePerioder.size - 1) {
        ...
        if (erIkkeSistePeriode && sortertePerioder[i].anvistPeriode.tom.plusDays(1) != sortertePerioder[i + 1].anvistPeriode.fom) {
            return Either.left(
                vedtakSammenlikningsFeilMetered(
                    SammenlikningsFeilÅrsak.INFOTRYGD_HULL_I_VEDTAKSPERIODE,
        ...
        if (erIkkeSistePeriode && sortertePerioder[i].utbetalingsgrad != sortertePerioder[i + 1].utbetalingsgrad) {
            return Either.left(
                vedtakSammenlikningsFeilMetered(
                    SammenlikningsFeilÅrsak.INFOTRYGD_FLERE_GRADERINGER_I_VEDTAK,
    ...
    return Either.right(
        InfotrygdVedtak(
            AnvistPeriode(
                fom = sortertePerioder.first().anvistPeriode.fom,
                tom = sortertePerioder.last().anvistPeriode.tom
            ), utbetalingsgrad = sortertePerioder.first().utbetalingsgrad
        )
    )
}
...
fun sammenliknPeriode(grunnlag: PeriodeSammenlikningsGrunnlag): Either<VedtaksSammenlikningsFeil, VedtaksSammenlikningsMatch> {
    val infotrygdGradertDagsats =
        graderDagsats(grunnlag.infotrygdDagsats100, grunnlag.infotrygdVedtak.utbetalingsgrad ?: 0)
...
fun årsinntekt(arbeidsforholdListe: List<Arbeidsforhold>): Long = // TODO: metrics for antall arbeidsforhold
